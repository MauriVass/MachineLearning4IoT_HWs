...
parser.add_argument('--version', type=str, choices=['1','2','3'], required=True, help='Model version: 1,2,3')
...
class Model:
	def __init__(self,model_version,model_type,frame_length,frame_step,mfcc,num_mel_bins,num_coefficients,train_ds,image_size=32,alpha=1,sparsity=None):
	...
	def DSCNNmodel(self):
		...
		model = keras.Sequential([
			keras.layers.Conv2D(filters=int(256*multiplier),kernel_size=[3,3],strides=self.strides,use_bias=False), #input_shape=(32,32,1)
			keras.layers.BatchNormalization(momentum=0.1),
			keras.layers.Activation('relu'),
...
save_best = True
model = Model(model_version,model,frame_length=int(frame_length),frame_step=int(frame_step),mfcc=mfcc,train_ds=train_ds,num_mel_bins=int(num_mel_bins),num_coefficients=int(num_coefficients),image_size=image_size,alpha=alpha,sparsity=sparsity)
