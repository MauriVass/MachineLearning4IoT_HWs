...
parser.add_argument('--version', type=str, required=True, help='model version: a, b or c')
...
class Model:
	def __init__(self,model_type,frame_length,frame_step,mfcc,num_mel_bins,num_coefficients,train_ds,image_size=32,alpha=1,sparsity=None,version=''):
		self.frame_length = frame_length
		self.frame_step = frame_step
		self.image_size = image_size
		self.num_coefficients = num_coefficients
		...
		self.output = f'Group14_kws_{version}'
...
save_best = True
model = Model(model,frame_length=int(frame_length),frame_step=int(frame_step),mfcc=mfcc,train_ds=train_ds,num_mel_bins=int(num_mel_bins),num_coefficients=int(num_coefficients),image_size=image_size,alpha=alpha,sparsity=sparsity,version=version)
hist = model.Train(train_ds,val_ds,20)
loss, acc = model.Test(test_ds,save_best)
print('Accuracy test set: ',acc)
output_model = model.SaveModel(save_best)
model.model.summary()
